  ...
  return (
    <div className="h-user-wrap" ref={ref}>
      <button className="h-user-btn" onClick={() => setOpen(o => !o)}>
        ...
        <svg
          className={`h-chevron ${open ? 'h-chevron--open' : ''}`}
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2.5"
...
function highlight(text, query) {
  ...
  return text.split(regex).map((part, i) =>
    regex.test(part) ? (
      <mark key={i} className="h-sugg-mark">
        {part}
      </mark>
    ) : (
      part
    ...
    >
      <div className="h-sugg-left">
        ...
        <span className="h-sugg-meta">
          {product.brand && (
            <span className="h-sugg-brand">{highlight(product.brand.name, query)}</span>
          )}
      ...
      <span className="h-sugg-price">{formatPrice(product.price)} TND</span>
    </button>
  );
}
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      productApi
        .autocomplete(query.trim(), { signal: controller.signal })
        .then(res => {
        ...
        .catch(() => {
          if (!controller.signal.aborted) setGroups(EMPTY_GROUPS);
        });
    }, 200);
    return () => {
      clearTimeout(timer);
  ...
  const handleKeyDown = e => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (activeSugg >= 0) {
        navigate(`/product/${flatSugg[activeSugg].id}`);
        closeSugg();
        setQuery('');
      } else {
        submitSearch();
        ...
        <div className="h-navbar-top">
            ...
            {hasResults && (
              <div className="h-sugg">
                ...
                {groups.nameContains.length > 0 && (
                  <>
                    {groups.nameStart.length > 0 && (
                      <div className="h-sugg-section">{t('otherNameMatches')}</div>
                    )}
                    {groups.nameContains.map((s, i) => (
                      <SuggItem
                        key={s.id}
                        product={s}
                        flatIndex={nameContainsOffset + i}
                        activeSugg={activeSugg}
                        onSelect={handleSelect}
                        onHover={setActiveSugg}
                        query={query}
          ...
          <div className="h-navbar-actions">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="h-icon-badge" data-testid="nav-cart-count">
                  {cartCount}
                </span>
              )}
            </button>
