...
export default function ProductCard({ product }) {
  ...
  const meta = CATEGORY_META[parentName] || { gradient: 'linear-gradient(135deg, #185FA5, #042C53)' };
    ...
    <div className="pc-card" data-testid="product-card" onClick={() => navigate(`/product/${product.id}`)}>
      <div className={`pc-image ${image ? 'pc-image--photo' : ''}`} style={!image ? { background: meta.gradient } : undefined}>
        {(promo || product.isNew) && (
          <div className="pc-tags">
            {promo && <Badge tone="danger" variant="solid" size="sm">{t('promotion')}</Badge>}
            {product.isNew && <Badge tone="success" variant="solid" size="sm">{t('new')}</Badge>}
          </div>
        )}
        {image
          ? <img src={image} alt={product.name} className="pc-img" />
          : <span className="pc-emoji"><CategoryIcon size={32} /></span>
        }
      ...
      <div className="pc-body">
        <div className="pc-badges">
          <Badge tone="primary" variant="soft">{product.category?.name}</Badge>
          {product.brand && (
            <Badge tone="neutral" variant="soft" icon={product.brand.image && <img src={product.brand.image} alt={product.brand.name} className="pc-brand-logo" />}>
              {product.brand.name}
            </Badge>
          )}
        </div>
