...
export default function Profile() {
  ...
  const handlePassword = async e => {
    ...
    setPwErr('');
    if (!pwForm.current) {
      setPwErr(t('password.errors.currentRequired'));
      return;
    }
    if (pwForm.next.length < 8) {
      setPwErr(t('password.errors.tooShort'));
      return;
    }
    if (pwForm.next !== pwForm.confirm) {
      setPwErr(t('password.errors.mismatch'));
      return;
    }
  ...
  const handleAddressSave = async addr => {
    try {
      if (mapModal.mode === 'add') {
        ...
        setAddresses(prev =>
          prev.map(a =>
            a.id === mapModal.addr.id ? res.data : addr.isDefault ? { ...a, isDefault: false } : a,
          ),
        );
      ...
      <main className="pf-main">
        <div className="pf-container">
          ...
          <div className="pf-header">
            ...
            <div>
              ...
              {user?.roles?.includes('ROLE_ADMIN') && (
                <span className="pf-admin-badge">{t('header.admin')}</span>
              )}
            </div>
          </div>
          ...
          <div className="pf-grid">
            <div className="pf-card">
              ...
              <label className="pf-checkbox-row">
                <input type="checkbox" checked={marketingOptIn} onChange={handleMarketingToggle} />
            ...
            <div className="pf-card pf-card--danger">
              ...
              <p className="pf-danger-text">{t('dangerZone.text')}</p>
          ...
          <div className="pf-card pf-card--full">
            ...
            <p className="pf-pref-hint">{t('preferences.hint')}</p>
          ...
          <div className="pf-card pf-card--full">
            ...
            {loading ? (
              ...
              <div className="pf-addr-empty">
                <div className="pf-addr-empty-icon">
                  <MapPin size={28} />
                </div>
                    ...
                    >
                      ...
                      <div className="pf-addr-body">
                        <div className="pf-addr-top">
                          <span className="pf-addr-label">
                            {t(`addressModal.labels.${addr.label?.toLowerCase()}`, {
                              defaultValue: addr.label,
                            })}
                          </span>
                          {addr.isDefault && (
                            <span className="pf-addr-default-badge">{t('addresses.default')}</span>
                          )}
                        </div>
                        ...
                        {addr.lat != null && addr.lng != null && (
                          <p className="pf-addr-coords">
                            <MapPin size={12} className="pf-inline-icon" />{' '}
                      ...
                      <div className="pf-addr-actions">
                        {!addr.isDefault && (
                          <button
                            className="pf-addr-btn pf-addr-btn--default"
                        ...
                        <button
                          className="pf-addr-btn pf-addr-btn--edit"
                          onClick={() => setMapModal({ mode: 'edit', addr })}
                        >
                          {t('addresses.editButton')}
                        </button>
                        <button
                          className="pf-addr-btn pf-addr-btn--del"
      ...
      {deleteModalOpen && (
        <div className="pf-overlay" onClick={() => !deleting && setDeleteModalOpen(false)}>
          <div className="pf-confirm-modal" onClick={e => e.stopPropagation()}>
            ...
            <p>
              {t('dangerZone.confirmModal.textBefore')}{' '}
              <strong>{t('dangerZone.confirmModal.textBold')}</strong>.{' '}
            ...
            <div className="pf-confirm-actions">
              ...
              <button className="pf-btn-danger" onClick={handleDeleteAccount} disabled={deleting}>
                {deleting
                  ? t('dangerZone.confirmModal.scheduling')
