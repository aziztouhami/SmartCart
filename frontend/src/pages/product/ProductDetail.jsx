...
function StockBadge({ stock }) {
  ...
  if (stock <= 10)
    return <span className="pd-stock pd-stock--low">{t('stock.low', { count: stock })}</span>;
...
export default function ProductDetail() {
  ...
  const loadActivity = useCallback(() => {
    productApi
      .activity(id)
      .then(res =>
        setActivity({ viewingNow: res.data.viewingNow ?? 0, inCarts: res.data.inCarts ?? 0 }),
      )
      .catch(() => {});
  }, [id]);
  ...
  useEffect(() => {
    ...
    const trackView = isAuthenticated()
      ? interactionApi.track(id, 'view')
      : guestEventApi.track(id, 'view');
      ...
      <main className="pd-main">
        <div className="pd-container">
          ...
          <div className="pd-breadcrumb">
            ...
            <span>›</span>
            <button onClick={() => navigate(`/?cat=${product.category?.id}`)}>
          ...
          <div className="pd-layout">
            ...
            <div className="pd-info">
              <div className="pd-badge-row">
                {product.promotion && (
                  <span className="pd-tag pd-tag--promo">{t('badges.promotion')}</span>
                )}
              ...
              <div className="pd-price-row">
                {product.promotion ? (
                  <>
                    <span className="pd-price pd-price--new">
                      {formatPrice(product.promotion.newPrice)}
                    </span>
                    <span className="pd-currency">TND</span>
                    <span className="pd-price-old">
                      {formatPrice(product.promotion.oldPrice)} TND
                    </span>
                  </>
                ) : (
                  <>
              ...
              {product.promotion && (
                <div className="pd-promo-meta">
                  <span className="pd-promo-pct">
                    {t('promo.off', { percentage: Math.round(product.promotion.percentage) })}
                  </span>
                  {product.promotion.endDate ? (
                    <span className="pd-promo-end">
                      {t('promo.ends', {
                        date: new Date(product.promotion.endDate).toLocaleDateString(
              ...
              {product.averageRating > 0 && (
                <div className="pd-rating-row">
                  ...
                  <span className="pd-rating-count">
                    {t('rating.reviewCount', { count: product.reviewCount })}
                  </span>
                </div>
              )}
              ...
              <div className="pd-meta">
                    ...
                    <span className="pd-meta-val pd-meta-val--brand">
                      {product.brand.image && (
                        <img
                          src={product.brand.image}
                          alt={product.brand.name}
                          className="pd-brand-logo pd-brand-logo--sm"
                        />
                      )}
                      {product.brand.name}
                    </span>
                ...
                <div className="pd-meta-row">
                  ...
                  <span className="pd-meta-val">
                    {product.stock > 0
                      ? t('meta.unitsAvailable', { count: product.stock })
                      : t('meta.unavailable')}
                  </span>
                </div>
              </div>
